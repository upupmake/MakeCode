...
- **详细报错**：若检测到语法错误，在工具结果中附加精确的行号和列号，最多展示前 3 个核心错误
- **不阻断写入**：语法告警不回滚已写入的内容。多步编辑的中间态本来可能不合法，阻断写入会让智能体卡死
...
- **整批原子编辑**：`FileEdit` 先在快照上定位全部编辑块并校验唯一性，暂时无法落位或与已接受块重叠的块推迟到下一阶段重试；只要有任何块始终无法落位即整批拒绝、不写入任何修改；全部落位后按倒序 splice 并原子替换文件。
...
- **全局上下文长度**：`memory_config.json` 中的 `context_length` 统一控制所有模型的上下文阈值，单位为千 tokens，默认 `350`；运行时每次使用前都会重新读取配置文件，超过阈值后自动压缩上下文。
...
- 文件读取、写入与编辑优先使用 File 工具；搜索默认使用终端命令（如 `grep`/`rg`/`find`）。
- 常规文件读写与编辑不应依赖终端命令完成。
